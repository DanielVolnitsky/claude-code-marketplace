...
- Consider all open reviews. Reuse a confident task match only within this repository and targeting its default branch; otherwise choose `dv-ai-factory/<task>`. Start from the latest `origin/<branch>` for a reused review or `origin/<default>` for a new task. Reject occupied branches/paths and conflicting local branches; never reset them.
